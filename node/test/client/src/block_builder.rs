...
use relay_template_test_runtime::{GetLastTimestamp, UncheckedExtrinsic};
...
impl InitPolkadotBlockBuilder for Client {
	...
	fn init_polkadot_block_builder_at(
		...
		let minimum_period = BasicExternalities::new_empty()
			.execute_with(|| relay_template_test_runtime::MinimumPeriod::get());
		...
		let slot_duration = BasicExternalities::new_empty()
			.execute_with(|| relay_template_test_runtime::SlotDuration::get());
